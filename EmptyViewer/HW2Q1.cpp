...
vec3 phongIllumination(const vec3& point, const vec3& normal, const Material& material, const vec3& viewDir) {
        ...
        for (int ix = 0; ix < Width; ++ix) {
            ...
            for (const auto& sphere : spheres) {
                ...
                if (sphere.intersect(ray, t) && t < min_t) {
                    ...
                    color = phongIllumination(point, normal, sphere.material, viewDir);
                }
            }
            ...
            if (plane.intersect(ray, t_plane) && t_plane < min_t) {
                ...
                vec3 viewDir = normalize(eye - point);
                color = phongIllumination(point, plane.normal, plane.material, viewDir);
            }
...
int main(int argc, char* argv[]) {
    ...
    window = glfwCreateWindow(Width, Height, "OpenGL Viewer(Q1. Phong effect - 202213827 투굴두루)", NULL, NULL);
